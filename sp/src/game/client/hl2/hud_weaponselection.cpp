...
private:
	...
	CHudTexture		*icon_buckets[ MAX_WEAPON_SLOTS ];
	CHudTexture		*icon_bucket_selection;
	Color			m_clrReddish;
};
...
void CHudWeaponSelection::VidInit()
{
	...
	icon_bucket_selection = gHUD.GetIcon( "bucket_selection" );
}
...
void CHudWeaponSelection::Paint()
{
	...
	for ( int i = 0; i < MAX_WEAPON_SLOTS; i++ )
	{
		...
		GetFgColor().GetColor( r1, g1, b1, a1 );

		int ypos = 10;

		icon_buckets[ i ]->DrawSelf( xpos, ypos, Color( r1, g1, b1, 255 ) );

		// INSOLENCE: If the current slot is the selected one, draw an extra circle icon above its number
		if ( i == iActiveSlot )
		{
			icon_bucket_selection->DrawSelf( xpos, ypos, Color( r1, g1, b1, 255 ) );
		}
		...
		else
		{
			...
			for ( int iPos = 0; iPos < MAX_WEAPON_POSITIONS; iPos++ )
			{
				...
				if ( pWeapon->HasAmmo() )
				{
					GetFgColor().GetColor( r2, g2, b2, a2 );
					a2 = 128;
				}
				else
				{
...
void CHudWeaponSelection::ApplySchemeSettings(vgui::IScheme *pScheme)
{
	...
	m_clrReddish	= pScheme->GetColor( "DamagedFg", Color( 255, 16, 16, 255 ) );
}
